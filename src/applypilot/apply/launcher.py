...
# ---------------------------------------------------------------------------
# LinkedIn Easy Apply flow (no DB dependency)
# ---------------------------------------------------------------------------

def _run_linkedin_job(job_url: str, config: dict, port: int, worker_id: int = 0,
                      model: str = "haiku", dry_run: bool = False) -> tuple[str, int]:
    """Apply to a LinkedIn job using simple Playwright automation.

    Returns:
        Tuple of (status_string, duration_ms). Status is one of:
        'applied', 'skipped', 'dry_run', or 'failed:reason'.
    """
    from applypilot.apply.simple_apply import apply_to_job
    ...
    try:
        ...
        profile_dir = str(config.get("_chrome_profile_dir", app_config.CHROME_WORKER_DIR / "worker-0"))

        # Apply to job using simple Playwright approach
        # Use the CDP port that the worker Chrome is running on
        cdp_port = port if port else (9222 + worker_id)
        result = apply_to_job(job_url, config, profile_dir, port=cdp_port)
        ...
        if result == "APPLIED":
            ...
            return (f"skipped:{result.split(':')[1]}", duration_ms)
        else:
            return (result, duration_ms)

    except Exception as e:
        logger.exception("Error in simple apply")
        duration_ms = int((time.time() - start) * 1000)
        return (f"failed:{str(e)[:30]}", duration_ms)


def run_linkedin_apply(job_urls: list[str], config_dict: dict,
                       model: str = "haiku", dry_run: bool = False,
                       headless: bool = False, workers: int = 1) -> dict:
    """Apply to LinkedIn jobs from a URL list (no DB dependency).

    Args:
        job_urls: List of LinkedIn job URLs to apply to.
        config_dict: Config dict with profile, answers, resume_path, etc.
        model: Claude model name.
        dry_run: Don't click Submit.
        headless: Run Chrome in headless mode.
        workers: Number of parallel workers (currently only 1 is supported).

    Returns:
        Dict with keys: applied (int), skipped (int), failed (int).
    """
    global POLL_INTERVAL
    _stop_event.clear()
    ...
    # Add APP_DIR to config for internal use
    config_dict["_app_dir"] = config.APP_DIR

    if not job_urls:
        console.print("[yellow]No LinkedIn jobs to apply to[/yellow]")
        return {"applied": 0, "skipped": 0, "failed": 0}

    console.print(f"[bold]LinkedIn Easy Apply[/bold] — {len(job_urls)} jobs")
    console.print(f"Dry run: {dry_run}, Model: {model}, Workers: {workers}")
    ...
    try:
        with Live(render_full(), console=console, refresh_per_second=2) as live:
            ...
            for idx, job_url in enumerate(job_urls):
                ...
                try:
                    console.print(f"\n[cyan]({idx + 1}/{len(job_urls)})[/cyan] {job_url}")
                    result, duration_ms = _run_linkedin_job(job_url, config_dict, port=port,
                                                            worker_id=0, model=model,
                                                            dry_run=dry_run)

                    if result == "applied":
                        applied_count += 1
                        update_state(0, jobs_applied=applied_count)
                    elif result == "dry_run":
                        # Treat dry_run as success for counting
                        applied_count += 1
                        update_state(0, jobs_applied=applied_count)
                    elif result.startswith("skipped"):
                        skipped_count += 1
                        update_state(0, jobs_done=applied_count + skipped_count + failed_count)
                    else:
                        failed_count += 1
                        update_state(0, jobs_failed=failed_count,
                                     jobs_done=applied_count + skipped_count + failed_count)
                    ...
                    failed_count += 1
                    add_event(f"Error: {str(e)[:40]}")

            refresh_thread.join(timeout=1)
            live.update(render_full())

        console.print(
            f"\n[bold]Done: {applied_count} applied, {skipped_count} skipped, "
            f"{failed_count} failed[/bold]"
        )
