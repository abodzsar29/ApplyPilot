...
def _kill_process_tree(pid: int) -> None:
    ...
    Uses graceful shutdown (SIGTERM) first, then forceful kill (SIGKILL)
    if needed to ensure Chrome can flush its databases (cookies, etc).
    """
    ...
    try:
        if platform.system() == "Windows":
            # Windows: try graceful close first, then force kill
            try:
                subprocess.run(
                ...
                time.sleep(2)  # Give it time to exit gracefully
            ...
            # Unix: SIGTERM first (graceful), then SIGKILL (forceful)
            try:
...
def setup_worker_profile(worker_id: int) -> Path:
    ...
    logger.info("[worker-%d] Profile dir: %s", worker_id, profile_dir)
    if (profile_dir / "Default").exists():
        logger.info("[worker-%d] Profile already exists, reusing", worker_id)
    ...
    for item in source.iterdir():
        ...
        try:
            ...
            logger.debug("[worker-%d] Skipped copying %s: %s", worker_id, item.name, e)

    logger.info("[worker-%d] Profile copied to %s", worker_id, profile_dir)
    return profile_dir
